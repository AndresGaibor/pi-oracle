...
function summarizeJob(jobId: string): string {
	...
	return [
		...
		typeof job.artifactFailureCount === "number"
			? `artifact-failures: ${job.artifactFailureCount}`
			: undefined,
		job.lastCleanupAt ? `last-cleanup: ${job.lastCleanupAt}` : undefined,
		job.cleanupWarnings?.length
			? `cleanup-warnings: ${job.cleanupWarnings.join(" | ")}`
			: undefined,
...
async function runAuthBootstrap(
	authWorkerPath: string,
	cwd: string,
): Promise<string> {
	const config = loadOracleConfig(cwd);
	try {
		await withGlobalReconcileLock(
			{ processPid: process.pid, source: "oracle_auth", cwd },
			async () => {
	...
	return await new Promise<string>((resolve, reject) => {
		const child = spawn(
			process.execPath,
			[authWorkerPath, JSON.stringify(config)],
			{
				cwd,
		...
		child.on("close", (code) => {
			const message =
				stdout.trim() ||
				stderr.trim() ||
				"Oracle auth bootstrap finished with no output.";
...
export function registerOracleCommands(
	pi: ExtensionAPI,
	authWorkerPath: string,
): void {
	pi.registerCommand("oracle", {
		description:
			"Ask the agent to prepare and dispatch a ChatGPT web oracle job",
	...
	pi.registerCommand("oracle-auth", {
		description:
			"Sync ChatGPT cookies from real Chrome into the oracle auth seed profile",
		handler: async (_args: string, ctx: ExtensionCommandContext) => {
			ctx.ui.notify(
				"Syncing ChatGPT cookies from real Chrome into the oracle auth seed profile…",
				"info",
			);
			try {
	...
	pi.registerCommand("oracle-status", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			if (explicitJobId && !readScopedJob(jobId, ctx.cwd)) {
				ctx.ui.notify(
					`Oracle job ${jobId} was not found in this project`,
					"warning",
				);
				return;
			}
	...
	pi.registerCommand("oracle-cancel", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			const job = explicitJobId
				? readScopedJob(jobId, ctx.cwd)
				: readJob(jobId);
			if (!job) {
				ctx.ui.notify(
					`Oracle job ${jobId} not found in this project`,
					"warning",
				);
				return;
			}
			if (!isActiveOracleJob(job)) {
				ctx.ui.notify(
					`Oracle job ${jobId} is not active (${job.status})`,
					"info",
				);
				return;
			}
	...
	pi.registerCommand("oracle-clean", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			const jobs =
				target === "all"
					? listJobsForCwd(ctx.cwd)
					: [readScopedJob(target, ctx.cwd)].filter(Boolean);
			if (jobs.length === 0) {
				ctx.ui.notify("No matching oracle jobs found", "warning");
				return;
			}

			const activeJobs = jobs.filter((job): job is NonNullable<typeof job> =>
				Boolean(job && isActiveOracleJob(job)),
			);
			...
			const removeJobs = async () => {
				for (const job of jobs) {
					...
					cleanupWarnings.push(
						...result.cleanupReport.warnings.map(
							(warning) => `${job.id}: ${warning}`,
						),
					);
				}
			};
			...
			try {
				await withGlobalReconcileLock(
					{ processPid: process.pid, source: "oracle_clean", cwd: ctx.cwd },
					async () => {
						await reconcileStaleOracleJobs();
						await removeJobs();
					},
			...
			const warningSuffix =
				cleanupWarnings.length > 0
					...
					: "";
			ctx.ui.notify(
				`Removed ${jobs.length} oracle job director${jobs.length === 1 ? "y" : "ies"}.${warningSuffix}`,
				cleanupWarnings.length > 0 ? "warning" : "info",
			);
		},
	});
}

...
function normalizeExpiration(expires: any) {
...
function normalizeDomain(domain: any, fallbackHost: string | undefined) {
  const raw = typeof domain === 'string' && domain.trim() ? domain.trim() : fallbackHost;
  if (!raw) return undefined;
  return raw.replace(/^\.+/, '').toLowerCase();
}

function allowedCookieHosts(chatUrl: string) {
...
function isAllowedCookieDomain(domain: string, chatUrl: string) {
...
export function normalizeImportedCookie(cookie: any, fallbackHost: string) {
...
export function classifyImportedCookie(cookie: any, chatUrl: string) {
...
export function filterImportableAuthCookies(cookies: any[], chatUrl: string) {
  const fallbackHost = (() => {
    try {
  ...
  const dropped: Array<{ cookie: any; reason: string }> = [];
...
export function ensureAccountCookie(cookies: any[], chatUrl: string) {
  const next = [...cookies];
